...
//@CrossOrigin({"http://localhost:5173"}) // 이거 하나면 모두 해결 됨!
@CrossOrigin("*") // 이거 하나면 모두 해결 됨!
@Slf4j	// simple log4j (log4j에서 주로 사용하는 기능을 모아뒀다!)
@RestController	// @Controller + @ResponseBody(=잭슨라이브러리동작)
@RequestMapping("/api")
public class HaniController {
	...
	@GetMapping("/idols")
	 public List<Idol> getIdols() {
		...
		// @RequestBody 그래서 자바객체인데도 화면에 띄우면 JSON 데이터처럼 브라우저에 보여줌
	}
	...
	public Idol updIdol(@RequestBody Idol uidol) {
		// 이 습관이 매우 중요하다! 개발자는 항상 값을 눈으로 봐야한다!
		log.debug("체킁{}", uidol);	// 체킁Idol(sid=20, name=이름수정했당, age=30)
...
//				log.debug("체킁{}", idol);
		...
		return sid + ""; // 해당 sid 를 가진 아이돌이 없는 경우
	}
